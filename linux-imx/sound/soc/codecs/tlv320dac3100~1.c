				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_err(codec->dev, "dapm: '%s' 0x%02x,%d %d\n",
			     ...
			     struct snd_soc_dai *dai)
{
	...
	dev_err(codec->dev, "## %s: width %d rate %d\n",
				  ...
				  int clk_id, unsigned int freq, int dir)
{
	...
	dev_err(codec->dev, "## %s: clk_id = %d, freq = %d, dir = %d\n",
...
static void aic31xx_clk_on(struct snd_soc_codec *codec)
{
	...
	dev_err(codec->dev, "codec clock -> on (rate %d)\n",
...
static int aic31xx_power_on(struct snd_soc_codec *codec)
{
	...
	dev_err(codec->dev, "aic31xx_power_on: %p\n", codec);
...
static int aic31xx_power_off(struct snd_soc_codec *codec)
{
	...
	dev_err(codec->dev, "aic31xx_power_off: %p\n", codec);
	regcache_cache_only(aic31xx->regmap, true);
...
{
	aic31xx->pdata.micbias_vg = 1; // MICBIAS_2_0V
	aic31xx->pdata.gpio_reset = 8; // MX6QDL_PAD_GPIO_8__GPIO1_IO08
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	dev_err(&i2c->dev, "## %s: %s codec_type = %d\n", __func__,
